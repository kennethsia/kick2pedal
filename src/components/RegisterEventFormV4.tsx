...
export function RegisterEventFormV4({ event, user }: RegisterEventFormProps) {
  ...
  const categoriesMap = {
    ...
    'Day 2 - K2P Runbike Racing International Championship': [
      ...
      { id: 16, name: '6 Years Mix ( Age 6.1-7.0 Years )' },
      { id: 42, name: '7-8 Years Mix ( Age 7.1 - 9.0 Years )' },
  ...
  const image =
    user.data.riderType === 'FILIPINO'
      ...
      : //@ts-expect-error asdasdas
        imageMap[event.title].FOREIGN;

  //@ts-expect-error asdasdas
  const categories = categoriesMap[event.title];
